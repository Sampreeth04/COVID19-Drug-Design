...
solvent_df = pd.read_csv('ouput_pubchem.csv')
final_df = pd.DataFrame(index = solvent_df['NAME'])
solute_df = pd.read_csv('./COVID_files/solute_df.csv')

#print(solute_df['Drug Name'].iloc[0])
...
class Dataclass(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        solute = solute_df['CANONICAL SMILE'].iloc[count]
...
for count in range(len(solute_df['Drug Name'])-1):
...
#valid_df['delta_g_cigin_v2_predicted(kcal/mol)']=out_list_cleaned
    final_df[solute_df['Drug Name'][count]] = out_list_cleaned
